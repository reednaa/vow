...
function writePad32(buf, offset, value) {
    for (let i = 31; i >= 0; i--) {
        buf[offset + i] = Number(value & 0xffn);
        value >>= 8n;
    }
}
function sameWitnessEvent(a, b) {
    if (a.mode !== b.mode)
        return false;
    if (a.mode === "ethereum" && b.mode === "ethereum") {
        return (a.event.emitter.toLowerCase() === b.event.emitter.toLowerCase() &&
            a.event.topics.length === b.event.topics.length &&
            a.event.topics.every((topic, index) => topic.toLowerCase() === b.event.topics[index].toLowerCase()) &&
            a.event.data.toLowerCase() === b.event.data.toLowerCase());
    }
    const solanaA = a;
    const solanaB = b;
    return (solanaA.event.programId.toLowerCase() === solanaB.event.programId.toLowerCase() &&
...
function encodeWitnessEvent(witness) {
    if (witness.mode === "ethereum") {
        return encodeEthereumEvent(witness.event.emitter, witness.event.topics, witness.event.data);
    }
    return encodeSolanaEvent(witness.event.programId, witness.event.discriminator, witness.event.data);
}
export function mergeWitnesses(witnesses) {
...
export function encodeVow(witnesses) {
