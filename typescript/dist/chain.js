...
function decodeSolanaGenesisHash(chainId, genesisHash) {
    let decoded;
    try {
        decoded = bs58.decode(genesisHash);
    }
    catch {
...
export function normalizeChainId(chainId) {
    ...
    const aliasedChainId = SOLANA_CHAIN_ID_ALIASES[clusterOrGenesisHash];
    if (aliasedChainId)
        return aliasedChainId;
    decodeSolanaGenesisHash(chainId, clusterOrGenesisHash);
    return `solana:${clusterOrGenesisHash}`;
}
export function caip2ToNumericChainId(chainId) {
